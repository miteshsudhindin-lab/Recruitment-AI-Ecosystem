...
\- \*\*Data Ingestion:\*\* Web scraping pipeline built with `requests` and `BeautifulSoup` to dynamically compile active roles into `scraped\_jobs.csv`.
...
\- `nb\_resume\_model.pkl` - Serialized Naive Bayes model.

\- `tfidf\_vectorizer.pkl` - Serialized text vectorizer.

\- `scraped\_jobs.csv` - Scraped job dataset.
